...
# ---- MUHIM TUZATISH ----
# Avvalgi kodda sessiya fayli FAQAT "agar u hali mavjud bo'lmasa" yozilar edi:
#   if SESSION_B64 and not os.path.exists(SESSION_FILE):
# Railway'da persistent Volume ishlatilsa, diskda eski (buzuq/band) sessiya
# fayli saqlanib qoladi va siz Environment Variables'da SESSION_B64_PART1/2'ni
# YANGILASANGIZ HAM, kod buni ko'rmay eski faylni ishlatishda davom etar edi —
# yangi sessiya yaratganingiz shu sabab hech narsani o'zgartirmas edi.
#
# Endi: agar SESSION_B64 berilgan bo'lsa, diskdagi eski fayl har doim
# o'chirilib, ENV'dagi eng so'nggi qiymatdan qayta yoziladi.
if SESSION_B64:
    if os.path.exists(SESSION_FILE):
        os.remove(SESSION_FILE)
        print("Diskdagi eski session fayli o'chirildi (ENV'dagi yangi qiymat ishlatiladi).")
    try:
        with open(SESSION_FILE, "wb") as f:
            f.write(base64.b64decode(SESSION_B64))
        print("Session fayl SESSION_B64'dan tiklandi.")
    except Exception as e:
        print(f"OGOHLANTIRISH: SESSION_B64'ni dekodlashda xato: {e}")
elif not os.path.exists(SESSION_FILE):
    print("OGOHLANTIRISH: SESSION_B64_PART1/2 (yoki SESSION_B64) topilmadi va "
          "diskda ham session fayl yo'q — userbot Telegram'ga kira olmaydi. "
          "generate_session.py'ni ishga tushirib, natijasini Railway "
